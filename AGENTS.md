...
- `pyproject.toml`: shared package, editable-install, Python path, and pytest configuration for both layers.
- `Fair Representation Act MMD/`: placeholder directory for future multi-member-district generation work. Do not add integration assumptions here until that work is explicitly brought into scope.
