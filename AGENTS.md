...
- `Simulation_Layer/Helpers/utils.py`
  ...
  - Math/round helpers:
    ...
    - `truncate_4(value)`
    - `append_round(rounds, election, status, totals, action, include_threshold, ballot_allocations=None)`
    - `count_votes_single_round(election, status, use_transfer_values) -> tuple[totals, ballot_allocations]`
    - `apply_threshold_to_elected(totals, status, threshold)`
    - `build_surplus_fractions(elected_candidate_ids, totals, threshold)`
    - `apply_simultaneous_surplus_transfer_values(election, round_ballot_allocations, surplus_fractions)`
