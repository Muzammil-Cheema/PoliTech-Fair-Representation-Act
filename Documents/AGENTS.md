...
6. Preserve deterministic metadata when exporting to simulation (`election_id`, `seat_count`, `mode`, `tie_break_order`, and the rank-cap field). `max_ranks_allowed` is the canonical handoff key; `max_rankings_allowed` is kept only as a compatibility alias. `tie_break_order` is an elimination priority: if tied candidates appear as `[A, B]`, `A` is eliminated before `B`.
...
- `Representational_Layer/orchestration.py`: compatibility wrapper for `Representational_Layer/Src/Representational_Layer/orchestration.py`.
...
- `Representational_Layer/Src/Representational_Layer/__init__.py`
  - Public exports for models, generation, scoring, orchestration, and input-contract validation.
- `Representational_Layer/Src/Representational_Layer/orchestration.py`
  ...
  - Composes scoring, ranking, ballot generation, and simulation-ready JSON export.
...
3. Ensure metadata includes:
  ...
  - optional `max_ranks_allowed` as the canonical rank-cap field, with `max_rankings_allowed` supported only as a compatibility alias
