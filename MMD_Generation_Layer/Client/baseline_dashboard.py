...
@st.cache_data
def load_shapefile(shape_path: str, id_col: str = ID_COLUMN) -> gpd.GeoDataFrame:
    ...
    if GEOM_COLUMN in gdf.columns and gdf.geometry.name != GEOM_COLUMN:
...
def plot_baseline_histogram(results_df: pd.DataFrame, output_dir: Path):
...
def main():
    ...
    if not shape_path.exists():
        st.error(f"❌ Shapefile not found at: {shape_path}")
        st.stop()
    ...
    with st.spinner("Loading data..."):
        ...
        gdf = load_shapefile(str(shape_path), ID_COLUMN)
    ...
    st.sidebar.metric("Democratic Seats", f"{selected_plan['dem_seats']}/{NUM_DISTRICTS}")
    st.sidebar.metric("Republican Seats", f"{selected_plan['rep_seats']}/{NUM_DISTRICTS}")
    ...
    with col1:
        ...
        with st.spinner("Rendering map..."):
            fig_map = create_district_map(gdf, assignment, int(plan_id))
            st.pyplot(fig_map)
            plt.close(fig_map)
    ...
    fig_hist, hist_output_path = plot_baseline_histogram(results_df, output_dir)
    st.pyplot(fig_hist)
    plt.close(fig_hist)
